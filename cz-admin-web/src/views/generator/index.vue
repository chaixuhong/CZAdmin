  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <el-input v-model="query.name" clearable size="small" placeholder="请输入表名" style="width: 200px;"
                  class="filter-item" @keyup.enter.native="crud.toQuery" />
        <rrOperation />
      </div>
      <crudOperation>
        <el-tooltip slot="right" class="item" effect="dark" content="数据库中表字段变动时使用该功能" placement="top-start">
          <el-button class="filter-item" size="mini" type="success" icon="el-icon-refresh" :loading="syncLoading"
                     :disabled="crud.selections.length === 0" @click="sync">同步</el-button>
        </el-tooltip>
      </crudOperation>
    </div>
    <!--表格渲染-->
      ...
      <el-table-column label="操作" width="160px" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button size="mini" style="margin-left: -1px;margin-right: 2px" type="text"
                     @click="toDownload(scope.row.tableName)">下载</el-button>
          <el-button size="mini" style="margin-left: -1px;margin-right: 2px" type="text">
...
import * as crudGenerator from '@/api/generator/generator'
...
export default {
  ...
  cruds () {
    return CRUD({ url: `${crudGenerator.model_url}/tables`, optShow: { reset: true } })
  },
  ...
  methods: {
    toGen (tableName) {
      // 生成代码
      const params = { name: tableName }
      crudGenerator.generatorCode(params).then(data => {
        this.$notify({
          title: '生成成功',
          type: 'success',
          duration: 2500
    ...
    toDownload (tableName) {
      // 打包下载
      const params = { name: tableName }
      crudGenerator.downloadCode(params).then(data => {
    ...
    sync () {
      ...
      crudGenConfig.syncColumns(tables).then(() => {
        this.crud.refresh()
