...
<template>
  ...
  <#if betweens??>
    <#list betweens as column>
      <#if column.queryType = 'BetWeen'>
        <date-range-picker v-model="query.${column.changeColumnName}" class="date-item" />
      </#if>
    </#list>
  </#if>
    ...
    <#if columns??>
      <#list columns as column>
        <#if column.formShow>
          <el-form-item label="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>"<#if column.istNotNull> prop="${column.changeColumnName}"</#if>>
            ...
            <el-radio v-model="form.${column.changeColumnName}" v-for="(item,index) in dict.${column.dictName}" :key="index" :label="item.value">{{ item.label }}</el-radio>
              <#else>
                未设置字典，请手动设置 Radio
              </#if>
            ...
            <el-select v-model="form.${column.changeColumnName}" filterable placeholder="请选择">
              <el-option
                v-for="(item,index) in dict.${column.dictName}"
                :key="index"
                :label="item.label"
                :value="item.value" />
            </el-select>
      ...
      <!--表格渲染-->
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler" :height="windowHeight-280">
        ...
        <el-table-column v-if="checkPer(['${changeClassName}:edit','${changeClassName}:del'])" label="操作" width="150px" align="center">
          <template slot-scope="scope">
            <udOperation
              :data="scope.row"
...
<script>
import * as crud${className} from '@/api/${changeClassName}'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '${apiAlias}', url: crud${className}.model_url, idField: '${pkChangeColName}', sort: '${pkChangeColName},desc', crudMethod: { ...crud${className} }})
  },
  data() {
    return {
      permission: {
        add: ['${changeClassName}:add'],
        edit: ['${changeClassName}:edit'],
        del: ['${changeClassName}:del']
      },
      rules: {
        <#if isNotNullColumns??>
