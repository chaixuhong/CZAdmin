      ...
      <el-col :xs="24" :sm="24" :md="10" :lg="11" :xl="11" style="margin-bottom: 10px">
        <el-card class="box-card">
          ...
          <el-table ref="table" v-loading="crud.loading" :data="crud.data" highlight-current-row style="width: 100%;"
            ...
            <el-table-column v-if="checkPer(['dict:edit','dict:del'])" label="操作" width="130px" align="center"
                             fixed="right">
      ...
      <el-col :xs="24" :sm="24" :md="14" :lg="13" :xl="13">
        <el-card class="box-card">
          <div slot="header" class="clearfix">
            <span>字典详情</span>
            <el-button v-if="checkPer(['dict:add']) && this.$refs.dictDetail && this.$refs.dictDetail.query.dictName"
                       class="filter-item" size="mini" style="float: right;padding: 4px 10px" type="primary"
                       icon="el-icon-plus" @click="$refs.dictDetail && $refs.dictDetail.crud.toAdd()">新增</el-button>
          </div>
...
import * as crudDict from '@/api/system/dict'
...
const defaultForm = { dictId: null, name: null, description: null, dictSort: 999 }
...
export default {
  ...
  cruds () {
    return [
      CRUD({ title: '字典', url: crudDict.model_url, idField: 'dictId', sort: 'dictSort', crudMethod: { ...crudDict } })
    ]
  },
